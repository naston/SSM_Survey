...
from src.models.s5.jax_compat import associative_scan
from src.models.s5.init import *
    ...
    def __init__(self, lambdaInit: torch.Tensor,
        ...
        match bcInit:
            ...
            case 'dense_columns' | 'dense':
                ...
                self.B = nn.Parameter(B_eigen_init(B_init, Vinv)((p, h), torch.float))
                if self.bidir:
            ...
            case 'factorized':
                ...
                self.BH = nn.Parameter(as_complex(init_columnwise_B((h, k, 2), torch.float32)))
                self.BP = nn.Parameter(as_complex(init_columnwise_B((p, k, 2), torch.float32)))
                self.CH = nn.Parameter(as_complex(init_rowwise_C((k, h, 2), torch.float32)))
                self.CP = nn.Parameter(as_complex(init_rowwise_C((k, cp, 2), torch.float32)))
...
class S5(nn.Module):
    def __init__(self,
        ...
        assert d_state % block_count == 0, "block_count should be a factor of state_width"

        block_size = d_state // block_count
